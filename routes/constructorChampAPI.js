...
router.get("/:name", (req, res) => {
    costructorDAO.getByName(req.params.name).then(costructor => {
...
router.post("/", (req, res) => {
    const {name, contry, wins, polePosition, podium, driverCampionship, constructorChampionship} = req.body

    //TODO validar os campos

    costructorDAO.save(name, contry, wins, polePosition, podium, driverCampionship, constructorChampionship).then(costructor => {
        res.json(sucess(costructor))
    }).catch(err => {
        console.log(err)
...
router.put("/:id", (req, res) => {
    ...
    if (constructorChampionship) obj.constructorChampionship = constructorChampionship

    if (obj == {}) {
        return res.status(500).json(fail("Nenhum atributo foi modificado"))
    }

    costructorDAO.update(id, obj).then(costructor => {
        if (costructor)
            res.json(sucess(costructor))
        else
            res.status(500).json(fail("Construtor não encontrada"))
    }).catch(err => {
        console.log(err)
...
router.delete("/:id", (req, res) => {
    costructorDAO.delete(req.params.id).then(costructor => {
        if (costructor)
            res.json(sucess(costructor))
        else
            res.status(500).json(fail("Construtor não encontrada"))
